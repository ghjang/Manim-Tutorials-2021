...
class testing(Scene):
    def construct(self):
        ...
        play_icon = (
            VGroup(*[SVGMobject(f"{HOME2}\\youtube_icon.svg") for k in range(8)])
            .set_height(0.75)
        ...
        def get_ball(x, y):
            dot = (
                Dot(fill_color=BLUE, fill_opacity=1)
        ...
        ball = always_redraw(
            lambda: get_ball(
                shift_req + l * np.sin(theta.get_value()),
        ...
        guest_name = Tex("Manoj Dhakal").next_to(
            vertical_line.get_start(), RIGHT, buff=0.5
        )
        guest_logo = (
            ImageMobject(f"{HOME}\\guest_logo.png")
            .set_width(2)
            .next_to(guest_name, DOWN, buff=0.1)
        )
        ...
        self.play(
            pendulum.animate.set_height(0.6).move_to(play_icon[7].get_center()),
            run_time=2,
        )
    ...
    def construct(self):
        ...
        graph = always_redraw(
            lambda: ParametricFunction(
                lambda u: np.array([4 * np.cos(u), 4 * np.sin(u), 0.5 * u]),
                color=BLUE,
                t_min=-3 * TAU,
                t_range=[-5, end.get_value()],
            )
        )
